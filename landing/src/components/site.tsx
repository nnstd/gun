...
export function SiteHeader({ current, crumb }: { current: PageKind; crumb?: ReactNode }) {
  return (
    <header className="sticky top-0 z-50 border-b border-white/10 bg-black/65 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1480px] flex-col gap-4 px-5 py-4 sm:px-8 lg:px-12">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-4">
            <a href="/" className="inline-flex items-center gap-3 text-white no-underline">
              <MascotMark size={32} />
              <span className="font-syne text-2xl font-extrabold tracking-[-0.04em]">gun</span>
            </a>
            {crumb ? (
              <div className="hidden items-center gap-3 text-sm text-white/45 lg:flex">
                <span>/</span>
                <span className="font-mono text-[13px] tracking-[0.08em] text-white/65">{crumb}</span>
              </div>
            ) : null}
          </div>
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:gap-8">
            <nav className="flex flex-wrap gap-x-5 gap-y-2 text-sm text-white/60 lg:justify-center">
              {NAV_ITEMS.map((item) => {
                const active = current === item.page && (item.page !== 'home' || item.href === '/#playground')
                return (
                  <a
                    key={item.label}
                    href={item.href}
        ...
        {crumb ? (
          <div className="font-mono text-[12px] tracking-[0.08em] text-white/60 lg:hidden">{crumb}</div>
        ) : null}
      </div>
    </header>
  )
...
export function SiteFooter() {
  return (
    <footer className="border-t border-white/10">
      <div className="mx-auto max-w-[1480px] px-5 py-12 sm:px-8 lg:px-12 lg:py-14">
        <div className="grid gap-10 border-b border-white/10 pb-10 lg:grid-cols-[1.4fr_1fr_1fr_1fr]">
          <div>
            <div className="mb-4 flex items-center gap-3">
              <MascotMark size={28} />
              <span className="font-syne text-2xl font-extrabold tracking-[-0.04em] text-white">gun</span>
            </div>
            <p className="max-w-xs text-sm leading-6 text-white/60">
              A JavaScript-to-Go compiler with a runtime that gets out of your way.
            </p>
          </div>
...
export function InstallTabs({ size = 'md' }: { size?: InstallTabSize }) {
  ...
  return (
    <div
      id="install"
      className={cx(
        'inline-flex w-full max-w-[420px] flex-col overflow-hidden rounded-2xl border border-brand-400/50 bg-black/55 shadow-[0_0_36px_rgba(66,68,147,0.22)]',
        large && 'max-w-[460px]',
      )}
    >
      <div className="grid grid-cols-4 border-b border-white/10 bg-brand-500/10">
        {(Object.keys(INSTALL_COMMANDS) as Array<keyof typeof INSTALL_COMMANDS>).map((tab) => (
          <button
            key={tab}
            type="button"
      ...
      <div className={cx('flex items-center gap-3 px-4 py-4', large && 'px-5 py-5')}>
        <span className="font-mono text-sm text-brand-300">$</span>
        <code className={cx('flex-1 overflow-x-auto font-mono text-sm text-white', large && 'text-[15px]')}>
          {command}
        </code>
        <button
          type="button"
...
export function Note({ type = 'info', children }: { type?: NoteKind; children: ReactNode }) {
  const styles = {
  ...
  return (
    <div className={cx('my-6 flex gap-4 rounded-2xl border px-4 py-4 text-sm leading-7', styles[type])}>
  ...
  return (
    <div className="my-6 overflow-hidden rounded-2xl border border-white/10 bg-black/55 shadow-[0_12px_42px_rgba(0,0,0,0.35)]">
      <div className="flex items-center gap-3 border-b border-white/10 bg-white/5 px-4 py-3">
        <span className={cx('rounded-md border px-2 py-1 font-mono text-[10px] font-bold uppercase tracking-[0.12em]', accents[lang] ?? accents.bash)}>
          {lang}
        </span>
        {file ? <span className="font-mono text-[12px] text-white/35">{file}</span> : null}
        <button
          type="button"
          onClick={copyCode}
  ...
  return (
    <div className="border-b border-white/10">
      ...
      >
        ...
        <span className={cx('flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-brand-300/45 text-white transition', open && 'rotate-180')}>
          <ChevronDown className="h-4 w-4" />
        </span>
      </button>
      {open ? <p className="max-w-3xl pb-6 pr-2 text-[15px] leading-7 text-white/65">{answer}</p> : null}
    </div>
  )
}

export function Mascot({ size = 280, compact = false }: { size?: number; compact?: boolean }) {
  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: glowSize, height: glowSize }}>
      ...
      >
        ...
        <ellipse cx="78" cy="62" rx="28" ry="16" fill="white" opacity="0.08" transform="rotate(-15 78 62)" />
        ...
        <path d="M82 120 Q100 136 118 120" stroke="#0f0f2e" strokeWidth="3.5" fill="none" strokeLinecap="round" />
...
export function MascotMark({ size = 28 }: { size?: number }) {
  return (
    ...
    >
      ...
      <circle cx="85" cy="88" r="4.5" fill="white" />
      <circle cx="129" cy="88" r="4.5" fill="white" />
      <path d="M82 120 Q100 136 118 120" stroke="#0f0f2e" strokeWidth="6" fill="none" strokeLinecap="round" />
    </svg>
  )
}
