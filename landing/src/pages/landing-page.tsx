...
import { FAQItem, InstallTabs, Mascot, SectionLabel, SiteFooter, SiteHeader } from '../components/site'

export const TRUSTED_BY = ['vercel', 'linear', 'stripe', 'figma', 'shopify', 'railway', 'planetscale', 'clerk']

export const BENCHMARK_ROWS = [
  { label: 'HTTP req/sec', node: '28k req/s', bun: '65k req/s', gun: '280k req/s', speedup: '10.0x' },
  { label: 'JSON parse 1MB', node: '8.4 ms', bun: '4.1 ms', gun: '1.2 ms', speedup: '7.0x' },
  { label: 'Cold start', node: '142 ms', bun: '64 ms', gun: '38 ms', speedup: '3.7x' },
  { label: 'Memory baseline', node: '64 MB', bun: '48 MB', gun: '22 MB', speedup: '2.9x' },
] as const
...
export const PILLARS = [
  {
    ...
    highlight: ['express@4.18.0', 'zod@3.22.0', 'drizzle-orm@0.29.0', '142 modules / 0 manual ports'],
  },
  {
    ...
    highlight: ['./build/api: ELF 64-bit', 'pie executable, x86-64', 'statically linked, stripped'],
  },
  {
  ...
  return (
    <div className="min-h-screen">
      ...
      <main>
        <section className="mx-auto grid max-w-[1480px] gap-14 px-5 pb-16 pt-16 sm:px-8 lg:grid-cols-[minmax(0,1.35fr)_minmax(380px,1fr)] lg:items-center lg:px-12 lg:pb-20 lg:pt-24">
          <div>
            <div className="mb-8 inline-flex items-center gap-3 rounded-full border border-brand-400/30 bg-brand-500/10 px-3 py-2 font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-brand-200">
              <span className="rounded-full bg-brand-500 px-2 py-1 text-white">New</span>
        ...
        <section id="playground" className="mx-auto max-w-[1400px] px-5 py-20 sm:px-8 lg:px-12 lg:py-24">
          ...
          <div className="mt-10 grid overflow-hidden rounded-[28px] border border-brand-400/30 bg-black/60 shadow-[0_24px_80px_rgba(0,0,0,0.45)] lg:grid-cols-[1fr_72px_1fr]">
            <CodePanel label="JavaScript" file="server.js" tone="amber" code={JS_SNIPPET} />
            ...
            <CodePanel label="Go" file="server.go" tone="cyan" code={GO_SNIPPET} />
          </div>
        </section>

        <section id="benchmarks" className="mx-auto max-w-[1480px] px-5 py-20 sm:px-8 lg:px-12 lg:py-24">
          ...
          <div className="mt-10 overflow-hidden rounded-[24px] border border-white/10 bg-panel">
            ...
            <div className="divide-y divide-white/10">
              {BENCHMARK_ROWS.map((row) => (
                <div key={row.label} className="grid gap-4 px-5 py-5 md:grid-cols-[2fr_1fr_1fr_1.2fr_0.8fr] md:px-7 md:py-6">
                  ...
                  <MetricCell label="Gun -> Go" value={row.gun} accent="brand" />
        ...
        <section id="examples" className="mx-auto max-w-[1480px] px-5 py-20 sm:px-8 lg:px-12 lg:py-24">
          <div className="max-w-3xl">
            <SectionLabel>03 / Anatomy</SectionLabel>
            <h2 className="text-balance font-syne text-[clamp(2.2rem,4vw,3.5rem)] font-extrabold leading-[1.02] tracking-[-0.05em] text-white">
              Five stages between <span className="text-amber-300">.js</span> and <span className="text-cyan-300">./bin</span>.
            </h2>
          ...
          <div className="mt-10 grid overflow-hidden rounded-[24px] border border-white/10 bg-panel lg:grid-cols-5">
            {STAGES.map((stage, index) => (
              <div key={stage.num} className="relative border-b border-white/10 px-6 py-7 last:border-b-0 lg:border-b-0 lg:border-r lg:last:border-r-0">
                <div className="font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-brand-300">{stage.num}</div>
                ...
                <h3 className="mt-5 font-syne text-2xl font-extrabold tracking-[-0.04em] text-white">{stage.title}</h3>
                <p className="mt-3 text-sm leading-7 text-white/60">{stage.description}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mx-auto max-w-[1480px] px-5 py-20 sm:px-8 lg:px-12 lg:py-24">
          ...
          <div className="mt-10 grid gap-4 lg:grid-cols-6">
            {PILLARS.map((pillar, index) => (
              <div
                key={pillar.title}
                className={index < 2 ? 'rounded-[24px] border border-white/10 bg-panel p-7 lg:col-span-3' : 'rounded-[24px] border border-white/10 bg-panel p-7 lg:col-span-2'}
              >
                <h3 className="font-syne text-[1.55rem] font-extrabold leading-[1.08] tracking-[-0.04em] text-white">
                  {pillar.title}
                </h3>
                <p className="mt-4 text-[15px] leading-7 text-white/60">{pillar.description}</p>
                {pillar.highlight ? (
        ...
        <section className="mx-auto max-w-[1480px] px-5 py-12 sm:px-8 lg:px-12 lg:py-16">
          <div className="grid gap-8 rounded-[28px] border border-brand-300/35 bg-[linear-gradient(135deg,rgba(66,68,147,0.18)_0%,rgba(66,68,147,0.04)_100%)] px-6 py-8 lg:grid-cols-[auto_1fr_auto] lg:items-center lg:px-12 lg:py-12">
            <div className="font-syne text-7xl font-extrabold leading-none text-brand-300/45">&ldquo;</div>
            <div>
              ...
              <div className="mt-6 flex items-center gap-4">
                ...
                <div>
                  <div className="text-sm font-semibold text-white">Sara Rahimi</div>
                  <div className="font-mono text-[12px] text-white/50">Staff Engineer / platform team</div>
                </div>
              </div>
            </div>
...
function CodePanel({
  ...
  return (
    <div className="min-w-0">
      ...
      <pre className="overflow-x-auto px-5 py-6 font-mono text-[12px] leading-7 text-white/90 sm:text-[13px]">
        <code>{code}</code>
      </pre>
    </div>
  )
...
function MetricCell({ label, value, accent = 'base' }: { label: string; value: string; accent?: 'base' | 'muted' | 'brand' }) {
  const bar = accent === 'brand' ? 'bg-brand-400' : accent === 'muted' ? 'bg-brand-300/70' : 'bg-white/25'

  return (
    <div>
      <div className="mb-2 font-mono text-[10px] font-bold uppercase tracking-[0.12em] text-white/35 md:hidden">{label}</div>
      <div className="mb-2 h-1.5 rounded-full bg-white/5">
        <div className={`${bar} h-full rounded-full`} />
      </div>
      <div className={accent === 'brand' ? 'font-mono text-[13px] font-semibold text-white' : 'font-mono text-[13px] text-white/60'}>{value}</div>
    </div>
  )
}
